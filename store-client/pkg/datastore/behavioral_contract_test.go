...
func (m *MockMaintenanceEventStore) FindEventsToTriggerQuarantine(ctx context.Context, gracePeriod time.Duration) ([]model.MaintenanceEvent, error) {
...
func (m *MockMaintenanceEventStore) FindEventsToTriggerHealthy(ctx context.Context, gracePeriod time.Duration) ([]model.MaintenanceEvent, error) {
	args := m.Called(ctx, gracePeriod)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockMaintenanceEventStore) UpdateEventStatus(ctx context.Context, eventID string, status model.InternalStatus) error {
...
func (m *MockHealthEventStore) UpdateHealthEventStatus(ctx context.Context, eventID string, status *datastore.HealthEventStatus) error {
	args := m.Called(ctx, eventID, status)
	return args.Error(0)
}
...
func TestMaintenanceEventStoreBehavior(t *testing.T) {
	...
	t.Run("FindEventsToTriggerQuarantine returns events", func(t *testing.T) {
		...
		store.On("FindEventsToTriggerQuarantine", ctx, gracePeriod).Return(expectedEvents, nil)

		events, err := store.FindEventsToTriggerQuarantine(ctx, gracePeriod)
	...
	t.Run("FindEventsToTriggerQuarantine handles empty results", func(t *testing.T) {
		...
		store.On("FindEventsToTriggerQuarantine", ctx, gracePeriod).Return([]model.MaintenanceEvent{}, nil)
...
func TestHealthEventStoreBehavior(t *testing.T) {
	t.Run("InsertHealthEvents success", func(t *testing.T) {
		store := &MockHealthEventStore{}
		ctx := context.Background()

		quarantined := datastore.Status("Quarantined")
		...
		store.On("InsertHealthEvents", ctx, events).Return(nil)

		err := store.InsertHealthEvents(ctx, events)
		assert.NoError(t, err)
		store.AssertExpectations(t)
	})

	t.Run("InsertHealthEvents handles empty list", func(t *testing.T) {
		store := &MockHealthEventStore{}
		ctx := context.Background()
		emptyEvents := []*datastore.HealthEventWithStatus{}

		store.On("InsertHealthEvents", ctx, emptyEvents).Return(nil)

		err := store.InsertHealthEvents(ctx, emptyEvents)
		assert.NoError(t, err, "Empty insert should succeed")
		store.AssertExpectations(t)
	})
	...
	t.Run("UpdateHealthEventStatusByNode updates by node", func(t *testing.T) {
		...
		ctx := context.Background()
		nodeName := "test-node"
		notQuarantined := datastore.Status("NotQuarantined")
		status := &datastore.HealthEventStatus{
			NodeQuarantined: &notQuarantined,
		}
	...
	t.Run("FindHealthEventsByStatus queries by status", func(t *testing.T) {
		store := &MockHealthEventStore{}
		ctx := context.Background()
		status := datastore.Status("Active")
...
func TestErrorHandlingConsistency(t *testing.T) {
	t.Run("Connection errors are identifiable", func(t *testing.T) {
		...
		connectionErr := datastore.WrapConnectionError(errors.New("connection refused"), "test-provider")
		assert.True(t, datastore.IsConnectionError(connectionErr),
			"Connection errors should be identifiable")
	})

	t.Run("Configuration errors are identifiable", func(t *testing.T) {
		configErr := datastore.WrapConfigurationError(errors.New("invalid config"), "test-provider", "invalid configuration setting")
		assert.True(t, datastore.IsConfigurationError(configErr),
			"Configuration errors should be identifiable")
	})

	t.Run("Validation errors are identifiable", func(t *testing.T) {
		validationErr := datastore.WrapValidationError(errors.New("invalid data"), "test-provider", "event")
		assert.True(t, datastore.IsValidationError(validationErr),
			"Validation errors should be identifiable")
	})

	t.Run("Certificate errors are identifiable", func(t *testing.T) {
		certErr := datastore.WrapCertificateError(errors.New("cert expired"), "test-provider", "certificate expired")
		assert.True(t, datastore.IsCertificateError(certErr),
			"Certificate errors should be identifiable")
	})

	t.Run("Timeout errors are identifiable", func(t *testing.T) {
		timeoutErr := datastore.WrapTimeoutError(errors.New("operation timed out"), "test-provider", "query")
		assert.True(t, datastore.IsTimeoutError(timeoutErr),
			"Timeout errors should be identifiable")
	})

	t.Run("Error unwrapping works correctly", func(t *testing.T) {
		originalErr := errors.New("original error")
		wrappedErr := datastore.WrapConnectionError(originalErr, "test-provider")
...
// TestHealthEventWithStatusContract verifies the HealthEventWithStatus structure
// is handled consistently across providers.
func TestHealthEventWithStatusContract(t *testing.T) {
	t.Run("HealthEventWithStatus structure is consistent", func(t *testing.T) {
		now := time.Now().UTC()
		quarantined := datastore.Status("Quarantined")
		...
		assert.NotNil(t, event.HealthEvent)
		assert.Equal(t, "test-node", event.HealthEvent.(*protos.HealthEvent).NodeName)
		assert.Equal(t, quarantined, *event.HealthEventStatus.NodeQuarantined)
	})

	t.Run("Nil HealthEvent is handled", func(t *testing.T) {
		event := &datastore.HealthEventWithStatus{
			CreatedAt: time.Now(),
			// HealthEvent is nil
		}

		// Should not panic when HealthEvent is nil
		assert.Nil(t, event.HealthEvent)
	})
}

// TestMaintenanceEventContract verifies the MaintenanceEvent structure
// is handled consistently across providers.
func TestMaintenanceEventContract(t *testing.T) {
	t.Run("MaintenanceEvent structure is consistent", func(t *testing.T) {
		now := time.Now().UTC()
		event := &model.MaintenanceEvent{
			EventID:                "event-123",
			NodeName:               "node-1",
			MaintenanceType:        model.MaintenanceType("ScheduledMaintenance"),
			CSP:                    model.CSP("aws"),
			Status:                 model.InternalStatus("Pending"),
			EventReceivedTimestamp: now,
		}

		// Verify all fields are accessible
		assert.Equal(t, "event-123", event.EventID)
		assert.Equal(t, "node-1", event.NodeName)
		assert.Equal(t, model.MaintenanceType("ScheduledMaintenance"), event.MaintenanceType)
		assert.Equal(t, model.CSP("aws"), event.CSP)
	})

	t.Run("MaintenanceEvent with timestamps", func(t *testing.T) {
		now := time.Now().UTC()
		...
		assert.True(t, event.ScheduledEndTime.After(*event.ScheduledStartTime),
			"Finish timestamp should be after start timestamp")
	})
}
...
func TestProviderBehaviorConsistency(t *testing.T) {
	...
	t.Run("Nil pointer checks", func(t *testing.T) {
		...
		store.On("UpsertMaintenanceEvent", ctx, (*model.MaintenanceEvent)(nil)).
			Return(datastore.WrapValidationError(errors.New("event cannot be nil"), "test-provider", "event"))

		err := store.UpsertMaintenanceEvent(ctx, nil)
		assert.Error(t, err, "Nil pointer should return error, not panic")
		assert.True(t, datastore.IsValidationError(err), "Should be a validation error")
		store.AssertExpectations(t)
	})
}
