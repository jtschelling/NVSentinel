...
func TestDataStoreInterfaceCompliance(t *testing.T) {
	...
	var _ datastore.DataStore = (*mongodb.MongoStore)(nil)
	...
	t.Run("MongoDB implements DataStore", func(t *testing.T) {
		...
		storeType := reflect.TypeOf((*mongodb.MongoStore)(nil))
		interfaceType := reflect.TypeOf((*datastore.DataStore)(nil)).Elem()

		assertImplementsInterface(t, storeType, interfaceType, "MongoDB MongoStore")
	})
...
func TestMaintenanceEventStoreInterfaceCompliance(t *testing.T) {
	// Compile-time interface compliance checks
	var _ datastore.MaintenanceEventStore = (*mongodb.MongoStore)(nil)
...
func TestHealthEventStoreInterfaceCompliance(t *testing.T) {
	// Compile-time interface compliance checks
	var _ datastore.HealthEventStore = (*mongodb.MongoStore)(nil)
...
func TestChangeStreamWatcherInterfaceCompliance(t *testing.T) {
	// Compile-time interface compliance checks
	var _ datastore.ChangeStreamWatcher = (*mongodb.ChangeStreamWatcher)(nil)
	var _ datastore.ChangeStreamWatcher = (*postgresql.PostgreSQLChangeStreamWatcher)(nil)

	t.Run("MongoDB implements ChangeStreamWatcher", func(t *testing.T) {
		watcherType := reflect.TypeOf((*mongodb.ChangeStreamWatcher)(nil))
		interfaceType := reflect.TypeOf((*datastore.ChangeStreamWatcher)(nil)).Elem()

		assertImplementsInterface(t, watcherType, interfaceType, "MongoDB ChangeStreamWatcher")
	})
...
func TestCommonMethodSignatures(t *testing.T) {
	t.Run("InsertMany has consistent signature", func(t *testing.T) {
		mongoType := reflect.TypeOf((*mongodb.MongoStore)(nil))
		pgType := reflect.TypeOf((*postgresql.PostgreSQLDataStore)(nil))

		mongoMethod, mongoFound := mongoType.MethodByName("InsertMany")
		pgMethod, pgFound := pgType.MethodByName("InsertMany")

		require.True(t, mongoFound, "MongoDB should have InsertMany method")
		require.True(t, pgFound, "PostgreSQL should have InsertMany method")

		// Compare signatures (excluding receiver)
		assert.Equal(t, mongoMethod.Type.NumIn(), pgMethod.Type.NumIn(),
			"InsertMany should have same number of input parameters")
		assert.Equal(t, mongoMethod.Type.NumOut(), pgMethod.Type.NumOut(),
			"InsertMany should have same number of output parameters")
	})
