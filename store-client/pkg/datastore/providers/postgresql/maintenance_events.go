...
func (p *PostgreSQLMaintenanceEventStore) UpsertMaintenanceEvent(
	...
	klog.V(2).Infof("Successfully upserted maintenance event: %s", event.EventID)
...
func (p *PostgreSQLMaintenanceEventStore) UpdateEventStatus(
	...
	klog.V(2).Infof("Successfully updated maintenance event status: %s -> %s", eventID, newStatus)
