...
const (
	// Label values applied by fault-quarantine:
	QuarantinedLabelValue NVSentinelStateLabelValue = "quarantined"

	// Label values applied by node-drainer:
	...
	// Label values applied by fault-remediation:
...
/*
The StateManager interface is leveraged by both node-drainer and fault-remediation to manage the
lifecycle of the dgxc.nvidia.com/nvsentinel-state node label. Note that fault-quarantine relies on its
...
Fault-remediation only consumes drain-succeeded; drain-failed nodes are not remediated.
