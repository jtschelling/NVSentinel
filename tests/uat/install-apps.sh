...
# Registry values file (for USE_LOCAL_BUILD mode - generated by build-push-images.sh)
# Works for both AWS ECR and GCP Artifact Registry
ECR_VALUES_FILE="${ECR_VALUES_FILE:-}"
...
log "  - CSP: $CSP"
log "  - ECR_VALUES_FILE: ${ECR_VALUES_FILE:-<not set>}"
...
install_nvsentinel() {
    ...
    if [[ "$CSP" == "aws" ]]; then
        ...
        log "Janitor-provider extra args not defined for: $CSP"
    fi
    ...
    # Add registry values file if using local build mode (overrides image repositories)
    # Works for both AWS ECR and GCP Artifact Registry
    if [[ -n "${ECR_VALUES_FILE:-}" ]] && [[ -f "$ECR_VALUES_FILE" ]]; then
        log "Using registry values file: $ECR_VALUES_FILE"
        helm_args+=("--values" "$ECR_VALUES_FILE")
    fi
